...
def get_number_of_generations_of_single_run(
    common_classes, population_factor=1, mutation_factor=0.1
):
    ...
    algorithm = HGTConjunctionAlgorithm(
        HGT_mutator, length, epsilon, performance_oracle, population_factor
    )
...
def HGT_simulations_main__performance_rate():
    ...
    number_of_activations, population_factors, HGT_factors, mutation_factors = (
        common_classes.get_simulation_variables()
    )
    HGT_factor = 0.5
    ...
    for _ in range(number_of_activations):
        ...
        avg_performance_rate[len(current_performance_rate)] = (
            ...
            [
                a + b
                for a, b in zip(performance_rate_with_same_generation[1], current_performance_rate)
            ],
        )
    ...
    for i in range(len(avg_performance_rate)):
        if avg_performance_rate[i][0] != 0:
            average_rate_for_generation_i = [
                avg_rate / avg_performance_rate[i][0] for avg_rate in avg_performance_rate[i][1]
            ]
            print(
                "performance rate for "
                + str(i)
                + " generation is: "
                + str(average_rate_for_generation_i)
            )
    ...
    for mutation_factor in mutation_factors:
        ...
        for _ in range(number_of_activations):
            generation_number += get_number_of_generations_of_single_run(
                common_classes, 1, mutation_factor
            )

        avg_number_of_generations = float(generation_number) / number_of_activations

        print(
            "number_of_generations for " + str(mutation_factor) + " mutation_factor "
...
def HGT_simulations_main__parallel():
    ...
    representation_class = common_classes.representation_class
